 ...
 */
export function createDataContext<I extends Identifier, T extends Database>(provider: AbstractProvider<I, T>): DataContext<I, T> {
	// biome-ignore lint/suspicious/noExplicitAny: The outer function enforces the type.
	const { CacheContext, useCache } = createCacheContext<ItemStore<I, T, any> | QueryStore<I, T, any>>();

	// If this provider is backed by an in-memory cache, pass it to the `ItemStore` and `QueryStore` instances we create.
	const memory = getSource<CacheProvider<I, T>>(CacheProvider, provider)?.memory;

	return {
		useItem: <K extends DataKey<T>>(collection: Nullish<K>, id: Nullish<I>): ItemStore<I, T, K> | undefined => {
			const cache = useCache() as Map<string, ItemStore<I, T, K>>;
		...
		useQuery: <K extends DataKey<T>>(collection: Nullish<K>, query: Nullish<ItemQuery<I, T[K]>>): QueryStore<I, T, K> | undefined => {
			const cache = useCache() as Map<string, QueryStore<I, T, K>>;
