import { InvalidFeedback, boolean, BooleanSchema } from "..";

// Tests.
describe("BooleanSchema()", () => {
	test("TypeScript", () => {
		// Test boolean.optional
		const s1: BooleanSchema<boolean> = boolean.optional;
		const r1: boolean = s1.validate(true);

		// Test boolean.required
		const s2: BooleanSchema<true> = boolean.required;
		const r2: true = s2.validate(true);

		// Test boolean({})
		const s3: BooleanSchema<boolean> = boolean({});
		const r3: boolean = s3.validate(true);
		const s4: BooleanSchema<true> = boolean({ required: true });
		const r4: true = s4.validate(true);
		const s5: BooleanSchema<boolean> = boolean({ required: false });
		const r5: boolean = s5.validate(true);
		const s6: BooleanSchema<boolean> = boolean({});
