import { InvalidFeedback, StringSchema } from "../index.js";
...
describe("StringSchema", () => {
	test("TypeScript", () => {
		...
		const schema1: StringSchema<string> = StringSchema.OPTIONAL;
		const schemaResult1: string = schema1.validate("ABC");

		// Test string.required
		const schema2: StringSchema<string> = StringSchema.REQUIRED;
		const schemaResult2: string = schema2.validate("ABC");

		// Test string({})
		const schema3: StringSchema<string> = StringSchema.create({ required: false });
		const schemaResult3: string = schema3.validate("ABC");
		const schema4: StringSchema<string> = StringSchema.create({});
		const schemaResult4: string = schema4.validate("ABC");
		const schema5: StringSchema<string> = StringSchema.create({});
		const schemaResult5: string = schema5.validate("ABC");
		const schema6: StringSchema<string> = StringSchema.create({ required: true });
		const schemaResult6: string = schema6.validate("ABC");

		// Test options on string({})
		const schema9: StringSchema<"abc"> = StringSchema.create({ options: ["abc"], required: true });
		const schema10: StringSchema<"abc" | ""> = StringSchema.create({ options: ["abc"] });
		const schema11: StringSchema<"abc"> = StringSchema.create({ options: { abc: "ABC" }, required: true });
		const schema12: StringSchema<"abc" | ""> = StringSchema.create({ options: { abc: "ABC" } });
		// @ts-expect-error Type cannot be a subset of `string` unless `options` key is set.
		const schema13: StringSchema<"abc"> = StringSchema.create({ required: true });
		// @ts-expect-error Type cannot be a subset of `string` unless `options` key is set.
		const schema14: StringSchema<"abc" | ""> = StringSchema.create({ required: false });
		// @ts-expect-error Error because `options.options` is missing so it doesn't match any overload.
		const schema15 = StringSchema.create<"abc" | "">({ required: false });
		// @ts-expect-error Error because `options.options` specifies an incorrect value.
		const schema16 = StringSchema.create<"abc" | "">({ required: false, options: ["def"] });
	});
	test("Constructs correctly", () => {
		const schema1 = StringSchema.create({});
		expect(schema1).toBeInstanceOf(StringSchema);
		expect(schema1.required).toBe(false);
		const schema2 = StringSchema.REQUIRED;
		expect(schema2).toBeInstanceOf(StringSchema);
		expect(schema2.required).toBe(true);
		const schema3 = StringSchema.OPTIONAL;
		expect(schema3).toBeInstanceOf(StringSchema);
		expect(schema3.required).toBe(false);
	});
	describe("validate()", () => {
	...
	describe("options.multiline", () => {
		...
		test("String with multiline keeps tab and line feed newline", () => {
			const schema1 = StringSchema.create({ multiline: true, trim: false });
	...
	describe("options.options", () => {
		...
		test("Value is allowed if it exists in array options", () => {
			const schema = StringSchema.create({ options: ["a", "b", "c"] });
		...
		test("Value is allowed if it exists in object options", () => {
			const schema = StringSchema.create({ options: { a: "A", b: "B", c: "C" } });
			expect(schema.validate("a")).toBe("a");
		});
