import { InvalidFeedback, NumberSchema } from "../index.js";

// Tests.
describe("NumberSchema", () => {
	test("TypeScript", () => {
		// Test number.optional
		const s1: NumberSchema<number | null> = NumberSchema.OPTIONAL;
		const r1: number | null = s1.validate(123);

		// Test number.required
		const s2: NumberSchema<number> = NumberSchema.REQUIRED;
		const r2: number = s2.validate(123);

		// Test number({})
		const s3: NumberSchema<number> = NumberSchema.create({ required: true });
		const r3: number = s3.validate(123);
		const s4: NumberSchema<number | null> = NumberSchema.create({ required: false });
		const r4: number | null = s4.validate(123);
		const s5: NumberSchema<number | null> = NumberSchema.create({ required: false });
		const r5: number | null = s5.validate(123);
		const s6: NumberSchema<number | null> = NumberSchema.create({});
		const r6: number | null = s6.validate(123);

		// Test options.
		const s9: NumberSchema<123> = NumberSchema.create({ options: [123], required: true });
		const s10: NumberSchema<123 | null> = NumberSchema.create({ options: [123] });
		const s11: NumberSchema<123> = NumberSchema.create({ options: { 123: "ABC" }, required: true });
		const s12: NumberSchema<123 | null> = NumberSchema.create({ options: { 123: "ABC" } });
		// @ts-expect-error Type cannot be a subset of `number` unless `options` key is set.
		const s13: NumberSchema<123> = NumberSchema.create({ required: true });
		// @ts-expect-error Type cannot be a subset of `number | null` unless `options` key is set.
		const s14: NumberSchema<123 | null> = NumberSchema.create({ required: false });
	});
	test("Constructs correctly", () => {
		const schema1 = NumberSchema.create({});
		expect(schema1).toBeInstanceOf(NumberSchema);
		expect(schema1.required).toBe(false);
		const schema2 = NumberSchema.REQUIRED;
		expect(schema2).toBeInstanceOf(NumberSchema);
		expect(schema2.required).toBe(true);
		const schema3 = NumberSchema.REQUIRED;
	...
	describe("options.value", () => {
		...
		test("Default value is applied correctly", () => {
			const schema = NumberSchema.create({ value: 1234 });
	...
	describe("options.required", () => {
		...
		test("Non-required value allows null", () => {
			const schema = NumberSchema.create({ required: false });
			expect(schema.required).toBe(false);
			expect(schema.validate(null)).toBe(null);
			expect(schema.validate("")).toBe(null);
		});
		test("Required value disallows null", () => {
			const schema = NumberSchema.create({ required: true });
	...
	describe("options.min", () => {
		...
		test("Min is checked correctly", () => {
			const schema = NumberSchema.create({ min: 100 });
	...
	describe("options.max", () => {
		...
		test("Max is checked correctly", () => {
			const schema = NumberSchema.create({ max: 100 });
	...
	describe("options.options", () => {
		...
		test("Value is allowed if it exists in options", () => {
			const schema = NumberSchema.create({ options: [1, 2, 3] });
		...
		test("Value is allowed if it exists in object options", () => {
			const schema = NumberSchema.create({ options: { "1": "A", "2": "B", "3": "C" } });
	...
	describe("options.unit", () => {
		const schema = NumberSchema.create({ unit: "meter" });
		test("Numbers pass through unchanged", () => {
			expect(schema.validate(1234)).toBe(1234);
			expect(schema.validate(19.99)).toBe(19.99);
		});
		...
		test("Base units are parsed correctly", () => {
			...
			expect(NumberSchema.create({ unit: "foot" }).validate("200ft")).toBe(200);
			expect(NumberSchema.create({ unit: "foot" }).validate("200 ft")).toBe(200);
			expect(NumberSchema.create({ unit: "foot" }).validate("200.99feet")).toBe(200.99);
			expect(NumberSchema.create({ unit: "foot" }).validate("200.99foot")).toBe(200.99);
			expect(NumberSchema.create({ unit: "foot" }).validate("200.99 foot")).toBe(200.99);
			expect(NumberSchema.create({ unit: "foot" }).validate("123,456 foot")).toBe(123456);
		});
		test("Other units are parsed correctly as base units", () => {
			expect(NumberSchema.create({ unit: "foot" }).validate("1yd")).toBe(3);
			expect(NumberSchema.create({ unit: "meter" }).validate("1km")).toBe(1000);
			expect(NumberSchema.create({ unit: "meter" }).validate("2ft")).toBe(0.6096);
			expect(NumberSchema.create({ unit: "meter" }).validate("2000ft")).toBe(609.6);
			expect(NumberSchema.create({ unit: "inch" }).validate("100 mi")).toBe(6336000);
			expect(NumberSchema.create({ unit: "millimeter" }).validate("1cm")).toBe(10);
			expect(NumberSchema.create({ unit: "millimeter" }).validate("1m")).toBe(1000);
			expect(NumberSchema.create({ unit: "millimeter" }).validate("1km")).toBe(1000000);
			expect(NumberSchema.create({ unit: "centimeter" }).validate("1m")).toBe(100);
			expect(NumberSchema.create({ unit: "centimeter" }).validate("1km")).toBe(100000);
		});
