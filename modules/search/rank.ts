 ...
 * - Target is array: call `rank()` recursively for each target and return the total relevance.
 * - Item is array: call `rank()` recursively for each item and return the total relevance.
...
export const rank: Ranker = (item, target) => {
	...
	if (target instanceof Array) {
		let relevance = 0;
		for (const t of target) relevance += rank(item, t);
		return relevance;
	}
	...
	if (item instanceof Array) {
		let relevance = 0;
		for (const i of item) relevance += rank(i, target);
		return relevance;
	}
