...
A wrapping provider for tests. `MockAPIProvider` intercepts fetches through a [`RequestHandler`](/util/http/RequestHandler) instead of hitting the network, and records every call it makes so a test can assert on them.
