...
`ValidationDBProvider`, `DebugDBProvider`, and `RecordingDBProvider` are all `ThroughDBProvider` subclasses.
...
Two-step is the `DBProvider` default, and `ThroughDBProvider` deliberately inherits it rather than passing the query through — the same theory as `transact()` below: a wrapper's behaviour should apply to everything that happens through it. `RecordingDBProvider` relies on this to record exactly which items a query write changed. Wrappers that don't need per-item behaviour override the query writes back to passthrough (calling `source` directly) to keep the engine's native efficiency and atomicity — `ValidationDBProvider` (validates the query write's inputs up front) and `DebugDBProvider` (logs the operation the caller made) both do this.

The derived reads (`DBProvider.requireItem()`, `DBProvider.getFirst()`, `DBProvider.requireFirst()`) are inherited the same way, so they route through the wrapper's own `getItem()` / `getQuery()` — e.g. `requireItem()` through a `ValidationDBProvider` returns validated data. Only the core operations delegate directly to `source`.

The per-item writes run concurrently (via `awaitValues()`, which settles every write before rejecting), so a two-step batch over a remote source costs one round-trip of latency rather than one per item — though the order the individual writes complete in (and appear in a `RecordingDBProvider` log) is not guaranteed within the batch. The resolve and the writes are separate steps, so a two-step query write is only atomic when it runs inside `transact()`.
...
The copy is made by the `cloneWith()` method and shares the wrapper's prototype and state. Override `cloneWith()` if a subclass holds per-instance mutable state that must not be shared with transaction copies — `RecordingDBProvider` does this to give each transaction its own log, so only a committed transaction's operations are merged into the main `operations` log.
