...
test("MemoryProvider: get queries", () => {
	...
	for (const { id, ...data } of basics) db.setItem("basics", id, data);
	// Equal queries.
	expectUnorderedKeys(db.getQuery("basics", { str: "aaa" }), ["basic1"]);
	expectUnorderedKeys(db.getQuery("basics", { str: "NOPE" }), []);
	expectUnorderedKeys(db.getQuery("basics", { num: 300 }), ["basic3"]);
	expectUnorderedKeys(db.getQuery("basics", { num: 999999 }), []);
	expectUnorderedKeys(db.getQuery("basics", { group: "a" }), ["basic1", "basic2", "basic3"]);
	expectUnorderedKeys(db.getQuery("basics", { group: "b" }), ["basic4", "basic5", "basic6"]);
	expectUnorderedKeys(db.getQuery("basics", { group: "c" }), ["basic7", "basic8", "basic9"]);
	// ArrayContains queries.
	expectUnorderedKeys(db.getQuery("basics", { "tags[]": "odd" }), ["basic1", "basic3", "basic5", "basic7", "basic9"]);
	expectUnorderedKeys(db.getQuery("basics", { "tags[]": "even" }), ["basic2", "basic4", "basic6", "basic8"]);
	expectUnorderedKeys(db.getQuery("basics", { "tags[]": "prime" }), ["basic1", "basic2", "basic3", "basic5", "basic7"]);
	expectUnorderedKeys(db.getQuery("basics", { "tags[]": "NOPE" }), []);
	// In queries.
	expectUnorderedKeys(db.getQuery("basics", { num: [200, 600, 900, 999999] }), ["basic2", "basic6", "basic9"]);
	expectUnorderedKeys(db.getQuery("basics", { str: ["aaa", "ddd", "eee", "NOPE"] }), ["basic1", "basic4", "basic5"]);
	expectUnorderedKeys(db.getQuery("basics", { num: [] }), []);
	expectUnorderedKeys(db.getQuery("basics", { str: [] }), []);
	// Sorting.
	...
	expectOrderedKeys(db.getQuery("basics", { $order: "id" }), keysAsc);
	expectOrderedKeys(db.getQuery("basics", { $order: "!id" }), keysDesc);
	expectOrderedKeys(db.getQuery("basics", { $order: "str" }), keysAsc);
	expectOrderedKeys(db.getQuery("basics", { $order: "!str" }), keysDesc);
	expectOrderedKeys(db.getQuery("basics", { $order: "num" }), keysAsc);
	expectOrderedKeys(db.getQuery("basics", { $order: "!num" }), keysDesc);
	// Combinations.
...
test("MemoryProvider: subscribing to documents", async () => {
	...
	const db = new MemoryProvider<TestCollections>();
	// Subscribe.
	const calls1: OptionalItem<BasicData>[] = [];
...
test("MemoryProvider: subscribing to filter query", async () => {
	...
	// Check end result.
	await runMicrotasks();
	expectUnorderedKeys(db.getQuery("basics", {}), ["basic6", "basic7"]);
});
