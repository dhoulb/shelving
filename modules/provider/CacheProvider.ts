...
export class CacheProvider extends ThroughProvider implements AsynchronousProvider {
	...
	private _cacheResult<T extends Data>(ref: DatabaseDocument<T>, result: Result<T>): Result<T> {
	...
	override async get<T extends Data>(ref: DatabaseDocument<T>): Promise<Result<T>> {
	...
	override async add<T extends Data>(ref: DatabaseQuery<T>, data: T): Promise<string> {
		const id = await super.add(ref, data);
		this.cache.set(ref.doc(id), data);
		return id;
	}

	override async set<T extends Data>(ref: DatabaseDocument<T>, data: T): Promise<void> {
	...
	private *_cacheEntries<T extends Data>(ref: DatabaseQuery<T>, entries: Entries<T>): Entries<T> {
	...
	// Override to cache any got results.
	override async getQuery<T extends Data>(ref: DatabaseQuery<T>): Promise<Entries<T>> {
	...
	override async setQuery<T extends Data>(ref: DatabaseQuery<T>, data: T): Promise<number> {
	...
	override async deleteQuery<T extends Data>(ref: DatabaseQuery<T>): Promise<number> {
