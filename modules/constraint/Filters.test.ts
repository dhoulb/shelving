...
test(".from()", () => {
	expect(Filters.from<{ a: number; b: number }>({ "a": 1, "!b": 1 })).toEqual(new Filters(new Filter("a", 1), new Filter("!b", 1)));
	expect(Filters.from<{ c: number[] }>({ "c[]": 1 })).toEqual(new Filters(new Filter("c[]", 1)));
	expect(Filters.from<{ a: number; b: number; c: number; d: number }>({ "a>": 1, "b>=": 1, "c<": 1, "d<=": 1 })).toEqual(new Filters(new Filter("a>", 1), new Filter("b>=", 1), new Filter("c<", 1), new Filter("d<=", 1)));
	expect(Filters.from<{ a: number; b: number }>({ "a": [1], "!b": [1] })).toEqual(new Filters(new Filter("a", [1]), new Filter("!b", [1])));
});
test(".filter()", () => {
