...
test(".from()", () => {
	// One sort order.
	expectOrderedKeys(Sorts.from<SortableEntity>("id").transform(allRand), ["a", "b", "c", "d"]);
	expectOrderedKeys(Sorts.from<SortableEntity>("!id").transform(allRand), ["d", "c", "b", "a"]);
	expectOrderedKeys(Sorts.from<SortableEntity>("second").transform(allRand), ["a", "b", "d", "c"]);
	expectOrderedKeys(Sorts.from<SortableEntity>("!second").transform(allRand), ["c", "d", "b", "a"]);
	// Two sort orders (where second is relevant).
	expectOrderedKeys(Sorts.from<SortableEntity>(["first", "id"]).transform(allRand), ["c", "d", "a", "b"]);
	expectOrderedKeys(Sorts.from<SortableEntity>(["!first", "id"]).transform(allRand), ["a", "b", "c", "d"]);
	expectOrderedKeys(Sorts.from<SortableEntity>(["first", "second"]).transform(allRand), ["d", "c", "a", "b"]);
	expectOrderedKeys(Sorts.from<SortableEntity>(["!first", "second"]).transform(allRand), ["a", "b", "d", "c"]);
	expectOrderedKeys(Sorts.from<SortableEntity>(["first", "!second"]).transform(allRand), ["c", "d", "b", "a"]);
	expectOrderedKeys(Sorts.from<SortableEntity>(["!first", "!second"]).transform(allRand), ["b", "a", "c", "d"]);
	// Two sort orders (but second isn't relevant).
	expectOrderedKeys(Sorts.from<SortableEntity>(["second", "first"]).transform(allRand), ["a", "b", "d", "c"]);
	expectOrderedKeys(Sorts.from<SortableEntity>(["!second", "first"]).transform(allRand), ["c", "d", "b", "a"]);
});
