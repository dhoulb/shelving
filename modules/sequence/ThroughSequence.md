...
Wraps a source `AsyncIterator` and presents it as a full [`Sequence`](/sequence/Sequence). Use it to turn a bare async iterator into a reusable `AsyncIterable`, and to guarantee `return()` and `throw()` are always available — delegating to the source when it provides them.
