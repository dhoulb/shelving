...
A `Query<T>` is a plain object that describes filters, ordering, and a result limit for a set of data items. These helpers parse, apply, and paginate queries — the same `Query` type that [db](/db) uses for all collection reads.
...
- [db](/db) — collection reads accept `Query<T>` directly
- [util](/util) — full util module overview, including `Updates`
