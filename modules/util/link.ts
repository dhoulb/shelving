 ...
 * - **Anything else** — relative ref (`./foo`, `../foo`, `foo`, `#anchor`, `?q`), protocol-relative URL (`//host/path`), or scheme-prefixed URI (`mailto:a@b`, `tel:123`, `https://x.com/y`) → fed to [`getBasedURI()`](/util/url/getBasedURI) with `url` as the base. Self-contained URIs ignore the base (their own scheme makes them absolute); protocol-relative and relative refs resolve against the page URL.
 *
 * Defaults:
 ...
 *
 * Bases are passed through to [`getURL()`](/util/url/getURL) / `getBasedURI()` lazily — neither `url` nor `root` is materialised into a normalised base until the chosen branch needs it.
 *
 ...
 *
 * Same classification and defaults as [`getLink()`](/util/link/getLink). Use when an absolute URI is required and there's no sensible "do nothing" path for the caller.
 *
