...
import { addDoc, collection, deleteDoc, doc, documentId, getCountFromServer, getDoc, getDocs, increment, limit, onSnapshot, orderBy, query, setDoc, updateDoc, where } from "firebase/firestore";
...
function _getItems<T extends Data>(snapshot: QuerySnapshot<T>): Items<T> {
...
export class FirestoreClientProvider<T extends Database> implements AsyncProvider<T> {
	...
	async updateQuery<K extends DataKey<T>>(c: K, q: ItemQuery<T[K]>, updates: Updates<T[K]>): Promise<void> {
		const snapshot = await getDocs(_getQuery(this._firestore, c, q));
