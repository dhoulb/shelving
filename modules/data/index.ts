...
/** Change: a change to a document's data (an explicit `undefined` for a prop indicates deletion). */
export type Change<T extends Data> = T | DeepPartial<T>;

/** Changes: a set of changes to multiple documents indexed by their ID (an explicit `undefined` for a document indicates deletion). */
