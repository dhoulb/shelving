import type { MutableObject, ReadonlyObject } from "shelving/tools";
import { Feedback, InvalidFeedback, isFeedback } from "shelving/feedback";
import { isObject } from "shelving/tools";
import { Schema, SchemaOptions } from "./Schema";
import { Validator } from "./Validator";

export type MapOptions<T extends ReadonlyObject> = SchemaOptions & {
 ...
 */
export class MapSchema<T extends ReadonlyObject> extends Schema<T> {
	readonly value: T;
...
export const map: {
	<T extends ReadonlyObject>(options: MapOptions<T>): MapSchema<T>;
	required<T extends ReadonlyObject>(items: Validator<T[string]>): MapSchema<T>;
	optional<T extends ReadonlyObject>(items: Validator<T[string]>): MapSchema<T>;
} = Object.assign(<T extends ReadonlyObject>(options: MapOptions<T>): MapSchema<T> => new MapSchema<T>(options), {
	required: <T extends ReadonlyObject>(items: Validator<T[string]>): MapSchema<T> => new MapSchema<T>({ items, required: true }),
	optional: <T extends ReadonlyObject>(items: Validator<T[string]>): MapSchema<T> => new MapSchema<T>({ items, required: false }),
});
