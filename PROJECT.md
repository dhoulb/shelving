...
- [x] **583** — Clean up and refactor the `docs/` scripts. Done: split into `docs/build.tsx` (orchestrator that calls `Bun.build` on `docs/render.tsx` and imports the bundled output), `docs/render.tsx` (the React tree and `renderApp` loop, bundled so `.module.css` imports resolve to hashed class-name objects matching the emitted stylesheet), `docs/env.ts` (config constants), and minimal `start.tsx` for the dev server. No more spawn-self process dance; everything runs in-process.
- [x] **671** — Multi-segment routing for the docs site. Done in `TreeApp.tsx`: the catch-all route is split into `"/"` (renders the root) and `"/{...path}"` (named multi-segment catchall captures the remainder as `path`, destructured and prefixed back into an absolute path for `<TreePage>`). Catchall placeholder support landed in `template.ts` (`{...name}` / `{name*}` / `**` etc., all equivalent).
- [x] **463** — Move the default tree element renderers (page, menu, card components for `tree-directory`, `tree-file`, `tree-documentation`) into the UI module as reusable defaults so every consumer doesn't have to define them from scratch. Done: card and page components live in `modules/ui/docs/` as default `mapping` entries on `<TreeCards>` and `<TreePage>`; menu items use a single `<TreeMenuItem>` fallback on `<TreeMenu>` (handles every `tree-*` element with optional nested expansion when proud).
