...
- [ ] **583** — Clean up and refactor the `docs/` scripts (`docs/build.tsx`, `docs/start.tsx`, `docs/docs.tsx`). Currently working but verbose — the two-pass bundle bootstrap is duplicated between `build.tsx` and `start.tsx`, the shared `docs.tsx` mixes config, extraction, and rendering. Goals: less duplication (extract the bootstrap into a shared helper), clearer separation of concerns, fewer moving pieces, simpler code paths.
- [ ] **264** — Render sidebar menu items as proper `<a>` links with active-state styling, hover/focus styles, and indented nesting for tree depth. Currently menu items are bare `<span>` elements (`modules/ui/docs/DirectoryMenuItem.tsx`, `FileMenuItem.tsx`) — they don't navigate when clicked. Compute hrefs from the element's `key` chain.
