import { MutableObject, isObject, ReadonlyObject } from "./object";
...
type Chunks = ReadonlyArray<Chunk>;

/** Template values in `{ placeholder: value }` format. */
export type TemplateValues = ReadonlyObject<string>;
 ...
 */
export const splitTemplate = (template: string): Chunks => (cache[template] ||= _splitTemplate(template));
const _splitTemplate = (template: string): Chunks => {
	...
	for (let i = 1; i < matches.length; i += 2) {
		...
		if (i > 1 && !pre.length) throw new SyntaxError("splitTemplate(): Placeholders must be separated by at least one character");
...
export const matchTemplate = (
	...
	const templates = lazy(lazyTemplates, target);
...
const getValue = (name: string, value: PlaceholderValues): string => {
	...
	if (value instanceof Function) return value(name);
	if (isObject(value)) {
		const v = value[name];
