...
import { splitTemplate, matchTemplate, renderTemplate, getPlaceholders } from ".";
...
describe("splitTemplate()", () => {
	test("Splits correctly", () => {
		...
		expect(splitTemplate(":a/:b")).toEqual([
			{ pre: "", placeholder: ":a", name: "a", post: "/" },
			{ pre: "/", placeholder: ":b", name: "b", post: "" },
		]);
		expect(splitTemplate("/:a/{b}/${c}/{{d}}/")).toEqual([
			{ pre: "/", placeholder: ":a", name: "a", post: "/" },
			{ pre: "/", placeholder: "{b}", name: "b", post: "/" },
			{ pre: "/", placeholder: "${c}", name: "c", post: "/" },
			{ pre: "/", placeholder: "{{d}}", name: "d", post: "/" },
		]);
		expect(splitTemplate("*")).toEqual([{ pre: "", placeholder: "*", name: "0", post: "" }]);
		expect(splitTemplate("*/*")).toEqual([
			{ pre: "", placeholder: "*", name: "0", post: "/" },
			{ pre: "/", placeholder: "*", name: "1", post: "" },
		]);
		expect(splitTemplate("/:a/{b}/*/*/")).toEqual([
			{ pre: "/", placeholder: ":a", name: "a", post: "/" },
			{ pre: "/", placeholder: "{b}", name: "b", post: "/" },
	...
	test("Errors if format is invalid", () => {
		expect(() => splitTemplate("{a}{b}")).toThrow(SyntaxError);
		expect(() => splitTemplate(":a{b}")).toThrow(SyntaxError);
		expect(() => splitTemplate("/:a{b}${c}{{d}}/")).toThrow(SyntaxError);
	});
});
