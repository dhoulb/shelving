...
const buildQuery = <T extends Data>(
	...
	for (const { type, key, value } of filters) query = query.where(key === "id" ? ID : key, FILTERS[type], value);
...
export class FirestoreClientProvider implements Provider {
	...
	reset(): Promise<void> {
		throw new Error("FirestoreClientProvider.reset(): Firestore database cannot be reset");
	}
}
